...
export const Home = () => {
  ...
  const { data, fetchNextPage, isLoading, isFetching } =
    useInfiniteArticleQuery({ perPage: 6, textSearch: debouncedValue });
  ...
  return (
    <div>
      <div className="block sm:container mx-auto lg:px-20 p-4 sticky top-[52px] lg:top-[76px] z-50 bg-white">
        <div className="lg:px-0 pb-2 lg:py-5 lg:pt-0">
          <input
            onChange={handleChange}
            className="appearance-none border-2 border-neutral-300 rounded-full w-full sm:w-3/5 py-3 px-4 text-gray-700 leading-tight focus:outline-none focus:bg-white focus:border-purple-500"
            placeholder="Search"
          />
        </div>
      </div>
      <div className="border-b lg:hidden">
        <div className="block sm:container mx-auto lg:px-10 p-4">
          <span className="text-md font-bold font-martelSans">
            Discover more of what matters to you
          </span>
          <div className="mt-3">
            {appConfig.topics.map((topic, idx) => (
              <div
                key={idx}
                className="px-4 py-2 rounded-full bg-neutral-200 inline-block mx-1 cursor-pointer first:ml-0 last:mr-0 mb-2"
              >
